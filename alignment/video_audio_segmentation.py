...
def do_segments(path, video_n, timestamps):
    ...
    for counter_segments, timestamp in enumerate(timestamps):
        ...
        print('Segment n.', counter_segments, ' starts at second ', time.strftime('%H:%M:%S', time.gmtime(start)),
              ' ends at ', time.strftime('%H:%M:%S', time.gmtime(end)))
...
def get_frame_rate(path, video_n):
    ...
    for file1 in os.listdir(path + r'\coordinates_and_timestamps'):
        #print(file1)
        # if (video_name in file1) and (file1.endswith('.npy')) and ("timestamp" in file1):
        #     print("In first loop")
        #     timestamp_file = np.load(file1)
    ...
    keypoints_dict = {'pose': pose_tensor,
                      ...
                      'right_hand_tensor': righthand_tensor}
...
def do_keypoints_segments(path, keypoints, video_n, segment, start, end, time_per_frame):
    start_frame = math.floor(time_per_frame * start)
    end_frame = math.floor(time_per_frame * end)
    ...
    for key, value in keypoints.items():
        ...
        # pose_tensor_final = keypoints['pose'][start_frame:end_frame, :, :]
        # face_tensor_final = keypoints['face'][start_frame:end_frame, :, :]
        # lefthand_tensor_final = keypoints['left_hand'][start_frame:end_frame, :, :]
        # righthand_tensor_final = keypoints['right_hand'][start_frame:end_frame, :, :]
...
def do_subtitles_segments(path, video_n, segment, start, end):
    ...
    print(alignedsub)
    with open(get_path(video_n, r'\{p}\subtitle_segments'.format(p=path), 'video{}_subtitle_' + segment + '.txt'), 'w') as f:
        f.write(alignedsub)
